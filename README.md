...
Furthermore, it eliminates the need for relative imports that traverse up the directory hierarchy using dot notation. Consequently, the second issue associated with import failures when the script is run as a standalone script, is effectively resolved.
